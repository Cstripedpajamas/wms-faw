layui.use(['form', 'admin', 'ax'], function () {
    ...
    //获取详情信息，填充表单
    var ajax = new $ax(Feng.ctxPath + "/wmsIntelligentCabinet2Conf/detail?id=" + Feng.getUrlParam("id"));
    var result = ajax.start();
    form.val('wmsIntelligentCabinet2ConfForm', result.data);

    //表单提交事件
    form.on('submit(btnSubmit)', function (data) {
        var ajax = new $ax(Feng.ctxPath + "/wmsIntelligentCabinet2Conf/editItem", function (data) {
            if (data.code == 200) {
                Feng.success("更新成功！");
