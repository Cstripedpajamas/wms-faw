layui.use(['form', 'admin', 'ax'], function () {
    ...
    $.ajax({
        url: Feng.ctxPath + "/apply/listSelectForMaterialTypeSpare",
        dataType: 'json',
        success: function (data) {
            ...
            $.each(data, function (index, item) {
                ...
                $('#scrapMaterialName').append(new Option(textMty,index));
            });
            form.render('select');
        }
    });
    ...
    form.on('select(scrapMaterialName)', function (data) {
        $.each(materialTypesList, function (index, item) {
            if (data.value == index) {
                var textArr=item.split(",");
                $('#scrapMaterialType').val(textArr[1]);
                $('#scrapMaterialSku').val(textArr[2]);
                $('#scrapMaterialId').val(index);
            }
        });
    });
