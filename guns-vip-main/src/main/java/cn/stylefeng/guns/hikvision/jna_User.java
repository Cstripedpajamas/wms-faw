 ...
 * @ClassName jna_test
 * @Description TODO
 * @Author ASD-FuBenHao 人脸一体机
 * @Date 2021/11/22 9:44
 * @Version 1.0
 **/
public class jna_User {
...
//        test01.CloseAlarmChan();//撤防
    ...
    //删除人员 根据工号
    public void DelUser(String strEmployeeID) {
        DelFaceInfo(strEmployeeID);//删除人脸图片(已经存在的工号关联的人脸图片)
        DelUserInfo(strEmployeeID);//删除人员信息(已经存在的工号)
    }

    //添加人员 根据工号 工号不能重复
    public void AddUser(String strEmployeeID, String name, String faceJpg) {
        try {
            AddUserInfo(strEmployeeID, name);
            AddFace(strEmployeeID,faceJpg);
            AddCard(strEmployeeID,strEmployeeID);
    ...
    //修改人员 根据工号
    public void EditUser(String strEmployeeID) {
        //修改人员，根据工号修改相关人员信息，必须是已经存在的工号，工号不支持修改（需要删除重新下发）
        try {
            ModifyUserInfo(strEmployeeID);
        } catch (UnsupportedEncodingException | InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    //查找人员 根据工号
    public void FindFaceInfo(String strEmployeeID) {
        //查询人脸
        SearchFaceInfo(strEmployeeID);
    }

    //批量添加人员人脸
    public void AllAddUser() {
        //举例批量下发两个工号关联的人脸图片
        String[] strID = {"1003", "1004"};
        String[] strName = {"李学亮A", "李学亮B"};
        String[] strJpg={"D:\\Hikvision\\fawFace\\pic\\FPID_[1002]_FacePic.jpg","D:\\Hikvision\\fawFace\\pic\\FPID_[1002]_FacePic.jpg"};
        String[] strCard={"1003","1004"};
        int strNum=2;
        try {
            AddMultiUserInfo(strID, strName, strNum);   //下发人脸需要先下发卡号或者使用已有卡号
            AddMultiFace(strID,strJpg, strNum); //下发人脸
            AddMultiCard(strID,strCard,strNum); //下发卡号
        } catch (UnsupportedEncodingException | InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public void GetAbility() {
        String strURL = "GET /ISAPI/AccessControl/capabilities?format=json";
...
//			jsonSearchCond.put("EmployeeNoList", EmployeeNoList);
    ...
    public void AddUserInfo(String strEmployeeID, String name) throws UnsupportedEncodingException, InterruptedException, JSONException {
        HCNetSDK.BYTE_ARRAY ptrByteArray = new HCNetSDK.BYTE_ARRAY(1024);    //数组
    ...
    public void AddFace(String strFPID, String jpgPath) throws JSONException, InterruptedException {
        HCNetSDK.BYTE_ARRAY ptrByteArray = new HCNetSDK.BYTE_ARRAY(1024);    //数组
        ...
        if (lHandler < 0) {
            ...
            int picdataLength = 0;
            try {
                picfile = new FileInputStream(new File(jpgPath));
    ...
    public void AddCard(String strFPID, String cardStr) throws JSONException, InterruptedException {
        HCNetSDK.BYTE_ARRAY ptrByteArray = new HCNetSDK.BYTE_ARRAY(1024);    //数组
        ...
        int lHandler = hCNetSDK.NET_DVR_StartRemoteConfig(lUserID, 2550/*NET_DVR_JSON_CONFIG*/, ptrByteArray.getPointer(), strInBuffer.length(), null, null);
        if (lHandler < 0) {
            System.out.println("AddCard NET_DVR_StartRemoteConfig 失败,错误码为" + hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("AddCard NET_DVR_StartRemoteConfig 成功!");
            //将中文字符编码之后用数组拷贝的方式，避免因为编码导致的长度问题
            String strInBuffer1 = "{\"CardInfo\":{\"employeeNo\":\"" + strFPID + "\",\"cardNo\":\"" + cardStr + "\",\"cardType\":\"normalCard\",\"leaderCard\":\"" +
                    "1" + "\",\"checkCardNo\":false}}";
            ...
            while (true) {
                ...
                if (dwState == -1) {
                    ...
                    System.out.println("下发卡片失败, json retun:" + jsonResult.toString());
                    break;
                } else if (dwState == HCNetSDK.NET_SDK_CONFIG_STATUS_EXCEPTION) {
                    System.out.println("下发卡片异常, json retun:" + jsonResult.toString());
                    break;
                } else if (dwState == HCNetSDK.NET_SDK_CONFIG_STATUS_SUCCESS) {//返回NET_SDK_CONFIG_STATUS_SUCCESS代表流程走通了，但并不代表下发成功，比如有些设备可能因为人员已存在等原因下发失败，所以需要解析Json报文
                    if (statusCode != 1) {
                        System.out.println("下发卡片成功,但是有异常情况:" + jsonResult.toString());
                    } else {
                        System.out.println("下发卡片成功: json retun:" + jsonResult.toString());
                    }
                    break;
                } else if (dwState == HCNetSDK.NET_SDK_CONFIG_STATUS_FINISH) {
                    //下发人员时：dwState其实不会走到这里，因为设备不知道我们会下发多少个人，所以长连接需要我们主动关闭
                    System.out.println("下发卡片完成");
                    break;
                }
            }
            if (!hCNetSDK.NET_DVR_StopRemoteConfig(lHandler)) {
            ...
            while (true) {
                ...
                if (dwState == -1) {
                    ...
                    if (numOfMatches != 0) {//确认有人脸
                        ...
                        String FPID = MatchList_1.getStr("FPID"); //获取json中人脸关联的工号

                        FileOutputStream fout;
                        try {
                            fout = new FileOutputStream("D:\\Hikvision\\fawFace\\FPID_[" + FPID + "]_FacePic.jpg");
                            //将字节写入文件
                            long offset = 0;
    ...
    public void AddMultiFace(String[] strFPID,String[] jpgStr, int iNum) throws JSONException, InterruptedException {
        HCNetSDK.BYTE_ARRAY ptrByteArray = new HCNetSDK.BYTE_ARRAY(1024);    //数组
        String strInBuffer = "POST /ISAPI/Intelligent/FDLib/FaceDataRecord?format=json ";
        System.arraycopy(strInBuffer.getBytes(), 0, ptrByteArray.byValue, 0, strInBuffer.length());//字符串拷贝到数组中
        ptrByteArray.write();
        ...
        if (lHandler < 0) {
            ...
            for (int i = 0; i < iNum; i++) {
                ...
                int picdataLength = 0;
                try {
                    picfile = new FileInputStream(new File(jpgStr[i]));
                } catch (FileNotFoundException e) {
                    e.printStackTrace();
                }
    ...
    public void AddMultiCard(String[] strFPID, String[] strCard, int iNum) throws JSONException, InterruptedException{
        HCNetSDK.BYTE_ARRAY ptrByteArray = new HCNetSDK.BYTE_ARRAY(1024);    //数组
        String strInBuffer = "PUT /ISAPI/AccessControl/CardInfo/SetUp?format=json";
        System.arraycopy(strInBuffer.getBytes(), 0, ptrByteArray.byValue, 0, strInBuffer.length());//字符串拷贝到数组中
        ptrByteArray.write();
        ...
        int lHandler = hCNetSDK.NET_DVR_StartRemoteConfig(lUserID, 2550/*NET_DVR_JSON_CONFIG*/, ptrByteArray.getPointer(), strInBuffer.length(), null, null);
        if (lHandler < 0) {
            System.out.println("AddMultiCard NET_DVR_StartRemoteConfig 失败,错误码为" + hCNetSDK.NET_DVR_GetLastError());
            return;
        } else {
            System.out.println("AddMultiCard NET_DVR_StartRemoteConfig 成功!");

            int iSend = 0;
            while (iSend < iNum) {
                //将中文字符编码之后用数组拷贝的方式，避免因为编码导致的长度问题
                String strInBuffer1 = "{\"CardInfo\":{\"employeeNo\":\"" + strFPID[iSend] + "\",\"cardNo\":\"" + strCard[iSend] + "\",\"cardType\":\"normalCard\",\"leaderCard\":\"" +
                        "1" + "\",\"checkCardNo\":false}}";
                int iStringSize = strInBuffer1.length();
    ...
    public void SetupAlarmChan(){
        if (lAlarmHandle < 0)//尚未布防,需要布防
        {
            ...
            m_strAlarmInfo.byRetAlarmTypeV40 =1;
    ...
    public void SetAcsCfg(){
        String strURL = "PUT /ISAPI/AccessControl/AcsCfg?format=json ";
        HCNetSDK.BYTE_ARRAY ptrUrl = new HCNetSDK.BYTE_ARRAY(BYTE_ARRAY_LEN);
        System.arraycopy(strURL.getBytes(), 0, ptrUrl.byValue, 0, strURL.length());
        ptrUrl.write();
        ...
        HCNetSDK.BYTE_ARRAY ptrInBuffer = new HCNetSDK.BYTE_ARRAY(ISAPI_DATA_LEN);
        ptrInBuffer.read();
        String strInbuffer = "{\"AcsCfg\":{\"uploadCapPic\": true,\"remoteCheckDoorEnabled\": true,\"checkChannelType\": \"PrivateSDK\",\"channelIp\" : \"192.168.26.117\"}}";
        ptrInBuffer.byValue = strInbuffer.getBytes();
        ptrInBuffer.write();
        ...
        if (!hCNetSDK.NET_DVR_STDXMLConfig(lUserID, struXMLInput, struXMLOutput)) {
            ...
            ptrStatusByte.read();
            String strOutXML = new String(ptrOutByte.byValue).trim();
            System.out.println("配置门禁参数开启远程核验功能 输出结果:" + strOutXML);
            String strStatus = new String(ptrStatusByte.byValue).trim();
            System.out.println("配置门禁参数开启远程核验功能 返回状态：" + strStatus);
        }
    }

    // 打开门
    public void OpenDor(int dorStr){
        boolean openKg=hCNetSDK.NET_DVR_ControlGateway(lUserID,dorStr,2);
        if (openKg){
