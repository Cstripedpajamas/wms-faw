...
import cn.stylefeng.guns.modular.onetypeservice.enums.AlarmColor;
...
public class OneTypeCabinetRenewController {
    ...
    @GetMapping("/toolVerify")
    @ApiOperation(value = "换新流程：2.查询工具内容")
    public ResponseData toolVerify(
            @ApiParam("工具条码") @RequestParam String materialSerialNumber,
            @ApiParam("任务编号") @RequestParam String taskNumber
    ) {
        WmsMaterialTool wmsMaterialTool = oneTypeCabinetService.findToolByMaterialSerialNumber(materialSerialNumber);
        if (wmsMaterialTool == null) {
            return ResponseData.error("工具维护中无此数据或被禁用");
        ...
        String materialSku = wmsMaterialTool.getMaterialSku();
        WmsCabinet1RenewTaskResult bySpec = wmsCabinet1RenewTaskService.findByTaskNumber(taskNumber);
        if (!Objects.equals(materialSku, bySpec.getMaterialSku())) {

            // 发送红色信号
            sendAlarmInfo(AlarmColor.RED);
            return ResponseData.error("工具类型不一致");
        }

        // 发送绿色信号
        sendAlarmInfo(AlarmColor.GREEN);
        return ResponseData.success(wmsMaterialTool);
    }
    ...
    public Declension sendAlarmInfo(AlarmColor color) {
        Map<String, String> params = new HashMap<>();
        params.put("State", "1");
        params.put("Color", color.toString().toLowerCase());
        params.put("Second", "15");
        return WmsApiService.sendAlarmMessage(params);
    }
